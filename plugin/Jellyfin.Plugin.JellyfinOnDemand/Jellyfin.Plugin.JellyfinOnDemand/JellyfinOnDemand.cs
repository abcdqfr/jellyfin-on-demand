    ...
    public class JellyfinOnDemand : BasePlugin<PluginConfiguration>, IHasWebPages
    {
        ...
        public static string BrandingDirectory
        {
            get
            {
                ...
                var pluginFolderName = Path.GetFileNameWithoutExtension(configPath) ?? "Jellyfin.Plugin.JellyfinOnDemand";
        ...
        internal string BuildScriptTag()
        {
            ...
            return $"<script plugin=\"{Name}\" version=\"{cacheKey}\" dev=\"{(devMode ? "true" : "false")}\" src=\"../JellyfinOnDemand/script?v={cacheKey}\" defer></script>";
        }
        ...
        private void CheckPluginPages(IApplicationPaths applicationPaths, IServerConfigurationManager serverConfigurationManager, int pluginPageConfigVersion)
        {
            ...
            if (!calendarExists && pluginConfig.CalendarPageEnabled && pluginConfig.CalendarUsePluginPages)
            {
                config.Value<JArray>("pages")!.Add(new JObject
                {
                    ...
                    { "Url", $"{(supportsSubUrls ? "" : rootUrl)}/JellyfinOnDemand/calendarPage" },
            ...
            if (!downloadsExists && pluginConfig.DownloadsPageEnabled && pluginConfig.DownloadsUsePluginPages)
            {
                config.Value<JArray>("pages")!.Add(new JObject
                {
                    ...
                    { "Url", $"{(supportsSubUrls ? "" : rootUrl)}/JellyfinOnDemand/downloadsPage" },
            ...
            if (!bookmarksExists && pluginConfig.BookmarksEnabled && pluginConfig.BookmarksUsePluginPages)
            {
                config.Value<JArray>("pages")!.Add(new JObject
                {
                    ...
                    { "Url", $"{(supportsSubUrls ? "" : rootUrl)}/JellyfinOnDemand/bookmarksPage" },
            ...
            // Discover: always offer via Plugin Pages when JellyfinOnDemand discovery is on
            ...
            if (!discoverExists && pluginConfig.JellyfinOnDemandDiscoveryEnabled)
            {
                config.Value<JArray>("pages")!.Add(new JObject
                {
                    { "Id", $"{namespaceName}.DiscoverPage" },
                    { "Url", $"{(supportsSubUrls ? "" : rootUrl)}/JellyfinOnDemand/discoverPage" },
                    { "DisplayText", "Discover" },
                    { "Icon", "explore" },
                    { "Version", pluginPageConfigVersion }
                });
            }
            else if (discoverExists && !pluginConfig.JellyfinOnDemandDiscoveryEnabled)
            {
            ...
            if (!hiddenContentExists && pluginConfig.HiddenContentEnabled && pluginConfig.HiddenContentUsePluginPages)
            {
                config.Value<JArray>("pages")!.Add(new JObject
                {
                    ...
                    { "Url", $"{(supportsSubUrls ? "" : rootUrl)}/JellyfinOnDemand/hiddenContentPage" },
