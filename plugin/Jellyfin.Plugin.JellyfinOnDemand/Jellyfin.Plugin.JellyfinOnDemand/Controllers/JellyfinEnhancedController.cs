...
using Jellyfin.Plugin.JellyfinOnDemand.Helpers.Jellyseerr;
using MediaBrowser.Model.Plugins;
using MediaBrowser.Model;
using MediaBrowser.Controller.Persistence;
using Jellyfin.Plugin.JellyfinOnDemand.Model.Arr;
using Jellyfin.Plugin.JellyfinOnDemand.Extensions;
...
namespace Jellyfin.Plugin.JellyfinOnDemand.Controllers
{
    ...
    public class JellyfinEnhancedController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetJellyseerrUserStatus()
        {
            ...
            {
                return Ok(new { active = true, userFound = true, reason = "jellyfin-on-demand" });
            }
                ...
                if (root.TryGetProperty("results", out var tmdbResults) && tmdbResults.ValueKind == JsonValueKind.Array)
                {
                    foreach (var item in tmdbResults.EnumerateArray())
                    {
                        ...
                        results.Add(new
                        {
                            ...
                            releaseDate,
                            firstAirDate,
                            // UNKNOWN — requestable in Seerr UI; JellyfinOnDemand remaps the button to Play.
        ...
        /// <summary>
        /// True when JellyfinOnDemand discovery is on and Seerr is not configured —
        /// discover/search chrome is backed by TMDB (ADR-004), not a Seerr process.
        /// </summary>
        private static bool IsJellyfinOnDemandTmdbMode(Configuration.PluginConfiguration? config)
            => config != null
               && config.JellyfinOnDemandDiscoveryEnabled
        ...
        public ActionResult GetPublicConfig()
        {
            ...
            return new JsonResult(new
            {
                ...
                // JellyfinOnDemand discovery configuration. Indexer URLs may contain
                ...
                config.JellyseerrEnabled,
                // JellyfinOnDemand discovery reuses the Seerr search chrome; never hide it
                // when discovery is on (saved JellyseerrShowSearchResults=false from ADR-004).
                JellyseerrShowSearchResults = config.JellyfinOnDemandDiscoveryEnabled || config.JellyseerrShowSearchResults,
        ...
        {
            var prefix = "Jellyfin.Plugin.JellyfinOnDemand.js.locales.";
            var suffix = ".json";
        ...
        public ActionResult GetLocale(string lang)
        {
            ...
            var resourcePath = $"Jellyfin.Plugin.JellyfinOnDemand.js.locales.{sanitizedLang}.json";
            ...
            if (stream == null && sanitizedLang.Contains('-'))
            {
                ...
                var fallbackPath = $"Jellyfin.Plugin.JellyfinOnDemand.js.locales.{baseLang}.json";
        ...
        {
            var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream($"Jellyfin.Plugin.JellyfinOnDemand.{resourcePath.Replace('/', '.')}");
        ...
        {
            var allowed = Jellyfin.Plugin.JellyfinOnDemand.Helpers.ArrUrlGuard.IsAllowedUrl(url);
        ...
        {
            var allowed = await Jellyfin.Plugin.JellyfinOnDemand.Helpers.ArrUrlGuard.IsAllowedUrlAsync(url, ct).ConfigureAwait(false);
            if (!allowed && !string.IsNullOrWhiteSpace(url))
            {
                _logger.Error($"IsAllowedUrl rejected outbound URL: {url}");
            }
                ...
                if (spState != null && (spState.Series.Count > 0 || spState.Movies.Count > 0 || spState.Collections.Count > 0))
                {
                    ...
                    foreach (var kvp in items.ToList())
                    {
                        ...
                        if (sanitizeTitleStreams && stripped.StreamData != null && !stripGenresEnabled)
                        {
                            ...
                            var clonedSd = new Jellyfin.Plugin.JellyfinOnDemand.Model.TagStreamData
                            {
                                ItemName = null,
                                ItemPath = null,
                                Streams = sd.Streams?.Select(st => new Jellyfin.Plugin.JellyfinOnDemand.Model.TagMediaStream
                                {
                                    Type = st.Type,
                                ...
                                Sources = sd.Sources?.Select(_ => new Jellyfin.Plugin.JellyfinOnDemand.Model.TagMediaSource
                                {
                                    Path = null,
                                    Name = null,
                                }).ToList(),
            ...
            try
            {
                ...
                if (results != null)
                {
                    ...
                    var enrichmentTasks = results.Select(async req =>
                    {
                        ...
                        {
                            avatarUrl = $"/JellyfinOnDemand/proxy/avatar?path={Uri.EscapeDataString(avatar)}";
                        }
