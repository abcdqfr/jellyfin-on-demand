  ...
  padding: 3px 0 1px;
    ...
    const fetchSessions = async () => {
        try {
            ...
            const resp = await fetch(ApiClient.getUrl('/JellyfinOnDemand/active-streams/sessions'), {
    ...
    const sendBroadcast = async (header, text, timeoutMs, resultEl) => {
        try {
            const token = ApiClient?.accessToken?.() || '';
            const resp = await fetch(ApiClient.getUrl('/JellyfinOnDemand/active-streams/broadcast'), {
                method: 'POST',
                headers: {
