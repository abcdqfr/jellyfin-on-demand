...
(function(JE) {
        ...
        const performFetch = async () => {
            ...
            try {
                const itemResult = await ApiClient.ajax({
                    type: 'GET',
                    url: ApiClient.getUrl(`/JellyfinOnDemand/watch-progress/${ApiClient.getCurrentUserId()}/${itemId}`),
                    dataType: 'json'
                });
    ...
    async function displayItemSize(itemId, container) {
        ...
        const performFetch = async () => {
            ...
            try {
                const itemResult = await ApiClient.ajax({
                    type: 'GET',
                    url: ApiClient.getUrl(`/JellyfinOnDemand/file-size/${ApiClient.getCurrentUserId()}/${itemId}`),
                    dataType: 'json'
                });
    ...
    function tmdbGet(path) {
        const url = ApiClient.getUrl(`/JellyfinOnDemand/tmdb${path}`);
    ...
    async function removeFromHomeSurface(itemId, surface, card) {
        ...
        try {
            await ApiClient.ajax({
                type: 'POST',
                url: ApiClient.getUrl(`/JellyfinOnDemand/${config.path}/hide/${itemId}`),
                data: '{}',
