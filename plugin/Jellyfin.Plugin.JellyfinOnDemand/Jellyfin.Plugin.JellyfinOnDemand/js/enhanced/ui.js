    ...
    JE.showEnhancedPanel = async () => {
        ...
        if (currentUserId) {
            try {
                ...
                const settingsResponse = await ApiClient.ajax({
                    type: 'GET',
                    url: ApiClient.getUrl(`/JellyfinOnDemand/user-settings/${currentUserId}/settings.json?_=${Date.now()}`),
                    dataType: 'json'
                });
        ...
        if (displayLanguageSelect) {
            ...
            (async () => {
                ...
                try {
                    const [localeCodes, cultures] = await Promise.all([
                        ApiClient.ajax({ type: 'GET', url: ApiClient.getUrl('/JellyfinOnDemand/locales'), dataType: 'json' }),
            ...
            displayLanguageSelect.addEventListener('change', async (e) => {
                ...
                // All dropdown options come from /JellyfinOnDemand/locales, so they are guaranteed valid
