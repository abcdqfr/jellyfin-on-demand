...
(function (JE) {
    ...
    JE.initializeReviewsScript = function () {
        ...
        function fetchReviews(tmdbId, mediaType) {
            ...
            const url = `${ApiClient.getUrl(`/JellyfinOnDemand/tmdb/${apiMediaType}/${tmdbId}/reviews`)}?language=en-US&page=1`;
            return fetch(url, {
                headers: {
        ...
        function fetchUserReviews(tmdbId, mediaType) {
            ...
            const url = ApiClient.getUrl(`/JellyfinOnDemand/reviews/${mediaType}/${tmdbId}`);
            return fetch(url, {
         ...
         * Backed by DELETE /JellyfinOnDemand/reviews/admin/{userIdN}/{mediaType}/{tmdbId},
        ...
        async function adminDeleteUserReview(targetUserId, tmdbId, mediaType) {
            ...
            const url = ApiClient.getUrl(`/JellyfinOnDemand/reviews/admin/${userIdN}/${mediaType}/${tmdbId}`);
            const response = await fetch(url, {
                method: 'DELETE',
                headers: { "Authorization": 'MediaBrowser Token="' + ApiClient.accessToken() + '"', "X-Emby-Token": ApiClient.accessToken() }
            });
