    ...
    try {
        ...
        if (JE && JE.jellyseerrAPI) {
            ...
            data = await ApiClient.ajax({
                type: 'GET',
                url: ApiClient.getUrl(`/JellyfinOnDemand/jellyseerr${endpoint}`),
                headers: { 'X-Jellyfin-User-Id': ApiClient.getCurrentUserId() },
                dataType: 'json'
            });
        }
...
async function checkForUnrequestedSeasons(data) {
    ...
    try {
        ...
        const response = await ApiClient.ajax({
            type: 'GET',
            url: ApiClient.getUrl(`/JellyfinOnDemand/jellyseerr/request?take=500&skip=0&filter=all`),
            headers: { 'X-Jellyfin-User-Id': ApiClient.getCurrentUserId() },
            dataType: 'json'
        });
...
// /JellyfinOnDemand/proxy/avatar proxy for CORS/auth) or an already-absolute URL (gravatar,
