...
(function(JE) {
    ...
    async function getPersonInfo(personId, signal) {
        ...
        try {
            ...
            const response = await fetchWithManagedRequest(`/JellyfinOnDemand/person/${personId}`, { signal });
    ...
    async function searchTmdbPerson(personName, signal) {
        ...
        try {
            ...
            const response = await fetchWithManagedRequest(
                `/JellyfinOnDemand/tmdb/search/person?query=${encodeURIComponent(personName)}`,
                { signal }
            );
    ...
    async function fetchPersonCredits(personId, signal) {
        try {
            ...
            const response = await fetchWithManagedRequest(
                `/JellyfinOnDemand/jellyseerr/person/${personId}/combined_credits`,
                { signal }
            );
