...
(function(JE) {
    ...
    async function fetchTmdbGenres(signal) {
        ...
        try {
            ...
            const [tvResponse, movieResponse] = await Promise.all([
                fetchWithManagedRequest('/JellyfinOnDemand/tmdb/genres/tv', fetchOptions).catch(() => []),
                fetchWithManagedRequest('/JellyfinOnDemand/tmdb/genres/movie', fetchOptions).catch(() => [])
            ]);
    ...
    async function getGenreInfo(genreId, signal) {
        ...
        try {
            ...
            const response = await fetchWithManagedRequest(`/JellyfinOnDemand/genre/${genreId}`, { signal });
    ...
    async function fetchTvDiscover(genreId, page = 1, signal) {
        try {
            ...
            let path = `/JellyfinOnDemand/jellyseerr/discover/tv/genre/${genreId}?page=${page}`;
    ...
    async function fetchMovieDiscover(genreId, page = 1, signal) {
        try {
            ...
            let path = `/JellyfinOnDemand/jellyseerr/discover/movies/genre/${genreId}?page=${page}`;
            if (sortBy) path += `&sortBy=${encodeURIComponent(sortBy)}`;
            const response = await fetchWithManagedRequest(path, { signal });
            if (signal?.aborted) {
                throw new DOMException('Aborted', 'AbortError');
