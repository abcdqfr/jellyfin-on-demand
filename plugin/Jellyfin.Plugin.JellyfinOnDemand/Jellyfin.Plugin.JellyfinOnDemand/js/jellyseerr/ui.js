...
(function(JE) {
    ...
    ui.addMainStyles = function() {
        ...
        style.textContent = `
            ...
            .jellyseerr-request-button.jellyseerr-button-jellyfin-on-demand-lucky { background-color: #0f766e !important; color: #fff !important; }
            .jellyseerr-request-button.jellyseerr-button-jellyfin-on-demand-lucky:hover:not(:disabled) { background-color: #0d9488 !important; transform: translateY(-2px); }
            .jellyseerr-request-button.jellyseerr-button-jellyfin-on-demand-library { background-color: #4338ca !important; color: #fff !important; }
            .jellyseerr-request-button.jellyseerr-button-jellyfin-on-demand-library:hover:not(:disabled) { background-color: #4f46e5 !important; transform: translateY(-2px); }
    ...
    function createJellyseerrSection(results = [], isJellyseerrOnlyMode, isJellyseerrActive, jellyseerrUserFound) {
        ...
        const onDemand = JE.pluginConfig?.JellyfinOnDemandDiscoveryEnabled !== false && !JE.pluginConfig?.JellyseerrEnabled;
        title.textContent = onDemand
            ? 'JellyfinOnDemand'
    ...
    async function fetchProviderIcons(container, tmdbId, mediaType) {
        ...
        const url = ApiClient.getUrl(`/JellyfinOnDemand/tmdb/${mediaType}/${tmdbId}/watch/providers`);
    ...
    function configureRequestButton(button, item, isJellyseerrActive, jellyseerrUserFound) {
        ...
        // JellyfinOnDemand: Play = ranked picker; Lucky = rank #1 → warm → real JF player.
        const onDemand = JE.pluginConfig?.JellyfinOnDemandDiscoveryEnabled !== false && !JE.pluginConfig?.JellyseerrEnabled;
        if (onDemand && item.mediaType !== 'collection') {
            ...
            button.className = 'jellyseerr-request-button jellyseerr-button-request jellyseerr-button-jellyfin-on-demand-play';
            button.onclick = async (e) => {
                e.preventDefault();
                e.stopPropagation();
                if (typeof JE.swarmShowReleasePicker !== 'function') {
                    if (typeof JE.toast === 'function') {
                        JE.toast('release picker not loaded', 4000);
                    }
                    return;
                }
            ...
            let lucky = actions.querySelector('.jellyseerr-button-jellyfin-on-demand-lucky');
            if (!lucky) {
                lucky = document.createElement('button');
                lucky.type = 'button';
                lucky.className = 'jellyseerr-request-button jellyseerr-button-jellyfin-on-demand-lucky';
            ...
            lucky.onclick = async (e) => {
                ...
                if (typeof JE.playFeelingLucky !== 'function') {
                    if (typeof JE.toast === 'function') {
                        JE.toast('lucky path not loaded', 4000);
                    }
                    return;
                }
                lucky.disabled = true;
            ...
            let library = actions.querySelector('.jellyseerr-button-jellyfin-on-demand-library');
            if (!library) {
                library = document.createElement('button');
                library.type = 'button';
                library.className = 'jellyseerr-request-button jellyseerr-button-jellyfin-on-demand-library';
            ...
            library.onclick = async (e) => {
                ...
                if (typeof JE.swarmAddToLibrary !== 'function') {
                    if (typeof JE.toast === 'function') {
                        JE.toast('add-to-library not loaded', 4000);
                    }
                    return;
                }
