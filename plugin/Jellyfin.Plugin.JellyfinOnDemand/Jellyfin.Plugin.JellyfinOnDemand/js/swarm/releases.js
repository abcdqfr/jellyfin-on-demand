(function (JE) {
    ...
    const logPrefix = '🪼 JellyfinOnDemand releases:';
    ...
    function ensurePickerStyles() {
        if (document.getElementById('jellyfin-on-demand-release-picker-styles')) return;
        const style = document.createElement('style');
        style.id = 'jellyfin-on-demand-release-picker-styles';
        style.textContent = `
            .jellyfin-on-demand-picker-backdrop { position:fixed; inset:0; background:rgba(0,0,0,.55); z-index:99998; display:flex; align-items:center; justify-content:center; padding:1rem; }
            .jellyfin-on-demand-picker { width:min(760px,100%); max-height:min(85vh,720px); overflow:auto; background:#1c1c1e; color:#f5f5f7; border-radius:10px; box-shadow:0 12px 40px rgba(0,0,0,.45); }
            .jellyfin-on-demand-picker header { padding:1rem 1.1rem .5rem; position:sticky; top:0; background:#1c1c1e; z-index:1; }
            .jellyfin-on-demand-picker h2 { margin:0; font-size:1.15rem; }
            .jellyfin-on-demand-picker .sub { opacity:.7; margin:.35rem 0 0; font-size:.9rem; }
            .jellyfin-on-demand-picker .filters { display:flex; flex-wrap:wrap; gap:.45rem; align-items:center; padding:.55rem 1.1rem .35rem; position:sticky; top:4.2rem; background:#1c1c1e; z-index:1; border-bottom:1px solid rgba(255,255,255,.08); }
            .jellyfin-on-demand-picker .filters label { font-size:.8rem; opacity:.8; display:flex; gap:.3rem; align-items:center; }
            .jellyfin-on-demand-picker .filters select, .jellyfin-on-demand-picker .filters input {
            ...
            .jellyfin-on-demand-picker .filters input[type="number"] { width:3.2rem; }
            .jellyfin-on-demand-picker .filters .count { margin-left:auto; font-size:.8rem; opacity:.65; }
            .jellyfin-on-demand-picker ul { list-style:none; margin:0; padding:.25rem 0 1rem; }
            .jellyfin-on-demand-picker li { margin:0 .75rem .45rem; padding:.7rem .85rem; border-radius:8px; background:rgba(255,255,255,.06); cursor:pointer; }
            .jellyfin-on-demand-picker li:hover, .jellyfin-on-demand-picker li:focus { background:rgba(86,215,255,.18); outline:none; }
            .jellyfin-on-demand-picker .meta { opacity:.75; font-size:.85rem; margin-top:.25rem; }
            .jellyfin-on-demand-picker .tag { display:inline-block; margin-right:.35rem; padding:.05rem .35rem; border-radius:4px; background:rgba(255,255,255,.1); font-size:.75rem; }
            .jellyfin-on-demand-picker .close { float:right; background:transparent; border:0; color:inherit; font-size:1.4rem; cursor:pointer; line-height:1; }
            .jellyfin-on-demand-picker .empty, .jellyfin-on-demand-picker .loading { padding:1.25rem; opacity:.8; }
            .jellyfin-on-demand-picker li.preselect { outline:1px solid rgba(86,215,255,.55); background:rgba(86,215,255,.12); }
            .jellyfin-on-demand-picker .ep-meta { opacity:.7; font-size:.8rem; }
            .jellyfin-on-demand-ep-table { width:100%; border-collapse:collapse; font-size:.88rem; margin:0 0 1rem; }
            .jellyfin-on-demand-ep-table th { position:sticky; top:0; background:#1c1c1e; text-align:left; padding:.5rem .7rem; cursor:pointer; user-select:none; opacity:.8; border-bottom:1px solid rgba(255,255,255,.14); white-space:nowrap; }
            .jellyfin-on-demand-ep-table th:hover, .jellyfin-on-demand-ep-table th:focus { opacity:1; outline:none; }
            .jellyfin-on-demand-ep-table th.sort-asc::after { content:' \u25B2'; }
            .jellyfin-on-demand-ep-table th.sort-desc::after { content:' \u25BC'; }
            .jellyfin-on-demand-ep-table td { padding:.45rem .7rem; border-bottom:1px solid rgba(255,255,255,.06); }
            .jellyfin-on-demand-ep-table td.file { max-width:26rem; overflow:hidden; text-overflow:ellipsis; }
            .jellyfin-on-demand-ep-table tbody tr { cursor:pointer; }
            .jellyfin-on-demand-ep-table tbody tr:hover, .jellyfin-on-demand-ep-table tbody tr:focus { background:rgba(86,215,255,.16); outline:none; }
            .jellyfin-on-demand-ep-table tbody tr.preselect { outline:1px solid rgba(86,215,255,.55); background:rgba(86,215,255,.1); }
            .jellyfin-on-demand-choice-row { display:flex; gap:.6rem; padding:0 1.1rem 1.1rem; }
            .jellyfin-on-demand-choice { flex:1; padding:.7rem; border-radius:8px; border:1px solid rgba(255,255,255,.15); color:inherit; cursor:pointer; font-size:.95rem; }
            .jellyfin-on-demand-choice.stream { background:#2c2c2e; }
            .jellyfin-on-demand-choice.cache { background:#0f766e; color:#fff; border-color:transparent; }
            .jellyfin-on-demand-choice:hover, .jellyfin-on-demand-choice:focus { filter:brightness(1.12); outline:none; }
        `;
        document.head.appendChild(style);
    }

    function closePicker() {
        document.getElementById('jellyfin-on-demand-picker-backdrop')?.remove();
    }
    ...
    function showEpisodePicker(release, ctx, filters, videoFiles) {
        return new Promise((resolve) => {
            ...
            backdrop.id = 'jellyfin-on-demand-picker-backdrop';
            backdrop.className = 'jellyfin-on-demand-picker-backdrop';
            backdrop.innerHTML = `
                <div class="jellyfin-on-demand-picker" role="dialog" aria-label="JellyfinOnDemand episodes">
                    <header>
                        <button type="button" class="close" aria-label="Close">&times;</button>
                        <h2>${esc(ctx.title || relTitle(release))}</h2>
                        <p class="sub">Pick an episode inside this release — click a column to sort</p>
                    </header>
                    <table class="jellyfin-on-demand-ep-table">
                        <thead><tr>
    ...
    async function playBindAndStart(release, ctx, filters, fileIndex, season, episode, fileIndexExplicit, batchFiles) {
        ...
        if (typeof JE.swarmShowWarmOverlay === 'function') {
            ...
            JE.toast(`warming swarm for ${relTitle(release)}…`, 8000);
        }
        let bind;
        try {
        ...
        if (!ready || !path) {
            ...
            if (typeof JE.toast === 'function') JE.toast(`${why}`, 7000);
            return;
        }
        ...
        if (typeof JE.toast === 'function') {
            JE.toast(`starting playback…`, 3000);
        }
        ...
        if (ok) {
            if (attempt.message && typeof JE.toast === 'function') {
                JE.toast(`${attempt.message}`, 5000);
            } else if (typeof JE.toast === 'function') {
                JE.toast(`playing ${title}`, 4000);
            }
            return;
        }
        ...
        if (reason === 'opened_item_details') {
            if (typeof JE.toast === 'function') {
                JE.toast('opened Jellyfin item — press Play in the normal player.', 6000);
            }
            return;
        }
        const detail = (attempt && attempt.message) || reason || 'unknown';
        if (typeof JE.toast === 'function') {
            JE.toast(`warm, but no Jellyfin playback (${detail})`, 9000);
        }
    }
    ...
    async function resolveFileSelection(release, ctx, filters) {
        ...
        if (isTv && typeof JE.swarm.listFiles === 'function') {
            if (typeof JE.toast === 'function') {
                JE.toast('reading torrent files…', 4000);
            }
    ...
    async function cacheBindAndStart(release, ctx, fileIndex, season, episode, fileIndexExplicit) {
        ...
        const btih = release.btih || release.Btih
            || (JE.swarmMagnet && JE.swarmMagnet.parse(release.magnet || release.Magnet));
        if (typeof JE.toast === 'function') {
            JE.toast(`starting library download for ${relTitle(release)}…`, 6000);
        }
        ...
        if (err) {
            ...
            if (typeof JE.toast === 'function') JE.toast(`${why}`, 8000);
            return;
        }
        ...
        if (typeof JE.toast === 'function') {
            JE.toast(`downloading "${title}" straight into your library…`, 6000);
        }
    ...
    function watchCacheProgress(btih, fileIndex, mediaType, title) {
        ...
        const tick = async () => {
            ...
            if (ready) {
                if (typeof JE.toast === 'function') JE.toast(`"${title}" is now in your library.`, 6000);
                return;
            }
            ...
            if (err) {
                ...
                if (typeof JE.toast === 'function') JE.toast(`library download stopped — ${why}`, 8000);
                return;
            }
    ...
    async function streamBindAndStart(release, ctx, fileIndex, season, episode, fileIndexExplicit) {
        ...
        const btih = release.btih || release.Btih
            || (JE.swarmMagnet && JE.swarmMagnet.parse(release.magnet || release.Magnet));
        if (!btih) {
            if (typeof JE.toast === 'function') JE.toast('no infohash for this release', 6000);
            return;
        }
        const streamUrl = typeof JE.swarmStreamUrl === 'function'
            ...
            : null;
        if (!streamUrl) {
            if (typeof JE.toast === 'function') JE.toast('could not build a stream URL', 6000);
            return;
        }
        if (typeof JE.toast === 'function') {
            JE.toast(`adding "${title}" to your library as a stream…`, 6000);
        }
        ...
        if (typeof JE.toast === 'function') {
            JE.toast(`"${title}" is now streamable from your library.`, 6000);
        }
    }
    ...
    function showStreamOrCacheChooser(ctx) {
        return new Promise((resolve) => {
            ...
            backdrop.className = 'jellyfin-on-demand-picker-backdrop';
            backdrop.innerHTML = `
                <div class="jellyfin-on-demand-picker" role="dialog" aria-label="JellyfinOnDemand add to library" style="width:min(420px,100%);">
                    <header>
                        <button type="button" class="close" aria-label="Close">&times;</button>
                    ...
                    <div class="jellyfin-on-demand-choice-row">
                        <button type="button" data-choice="stream" class="jellyfin-on-demand-choice stream">Stream (.strm)</button>
                        <button type="button" data-choice="cache" class="jellyfin-on-demand-choice cache">Cache to library</button>
                    </div>
                </div>`;
            document.body.appendChild(backdrop);
    ...
    JE.jellyfinOnDemandPlayLucky = async function (ctx) {
        ctx = ctx || {};
        if (typeof JE.toast === 'function') JE.toast('finding top match…', 4000);
        const release = await pickTopRelease(ctx);
        if (!release) {
            if (typeof JE.toast === 'function') JE.toast('no releases found', 5000);
            return;
        }
        if (JE.swarmHistory && typeof JE.swarmHistory.recordSearch === 'function') {
            JE.swarmHistory.recordSearch(ctx);
    ...
    JE.swarmShowReleasePicker = async function (ctx, mode) {
        ...
        const query = buildSearchQuery(ctx) || String(ctx.query || ctx.title || '').trim();
        if (!query) {
            if (typeof JE.toast === 'function') JE.toast('empty search query', 3000);
            return;
        }
        ...
        backdrop.className = 'jellyfin-on-demand-picker-backdrop';
        backdrop.innerHTML = `
            <div class="jellyfin-on-demand-picker" role="dialog" aria-label="JellyfinOnDemand releases">
                <header>
                    <button type="button" class="close" aria-label="Close">&times;</button>
        ...
        const data = await JE.swarm.searchTorznab(query);
        const panel = backdrop.querySelector('.jellyfin-on-demand-picker');
        const results = (data && data.results) || [];
        if (!results.length) {
