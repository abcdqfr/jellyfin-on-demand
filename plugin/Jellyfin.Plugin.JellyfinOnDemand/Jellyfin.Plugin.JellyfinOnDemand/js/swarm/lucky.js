(function (JE) {
     ...
     * flow (JE.jellyfinOnDemandPlayLucky in releases.js) so series/episode picks aren't silent. */
    async function playFeelingLucky(ctx) {
        if (typeof JE.swarmSetBatchSession === 'function') JE.swarmSetBatchSession(null);
        if (ctx && ctx.mediaType === 'tv' && typeof JE.jellyfinOnDemandPlayLucky === 'function') {
            return JE.jellyfinOnDemandPlayLucky(ctx);
        }
        ...
        if (typeof JE.toast === 'function') JE.toast('starting playback…', 3000);
        ...
        if (ok && attempt.message && typeof JE.toast === 'function') {
            ...
            JE.toast(`warm, but no Jellyfin playback (${(attempt && attempt.message) || (attempt && attempt.reason) || 'unknown'})`, 9000);
        }
    ...
    function streamUrl(bind) {
        ...
        if (!btih) return null;
        return ApiClient.getUrl('/JellyfinOnDemand/swarm/stream', {
            btih,
            fileIndex,
    ...
    async function fetchSessions() {
        try {
            ...
            console.warn('jellyfin-on-demand: Sessions list failed', e);
            return [];
        }
    }
    ...
    async function attemptPlayback(bind, title) {
        document.getElementById('jellyfin-on-demand-player-overlay')?.remove();
        document.getElementById('jellyfin-on-demand-player-styles')?.remove();
        // Discover (and any other JellyfinOnDemand custom pane) must leave the stack
        ...
        if (pm) {
            try {
                ...
                console.warn('jellyfin-on-demand: playbackManager.play(ids) failed', e);
            }
            ...
            try {
                ...
                console.warn('jellyfin-on-demand: playbackManager.play(items) failed', e);
            }
        }
        ...
        if (session) {
            try {
                ...
                console.warn('jellyfin-on-demand: Session PlayNow failed', e);
                return {
                    ok: false,
        ...
        if (window.Emby?.Page?.showItem) {
            ...
            if (playBtn && !playBtn.disabled) {
                try {
                    ...
                    console.warn('jellyfin-on-demand: detail Play click failed', e);
                }
            }
            return {
                ok: false,
    ...
    const WARM_OVERLAY_ID = 'jellyfin-on-demand-warm-overlay';
            ...
            try {
                ...
                if (el) {
                    ...
                    if (Number.isFinite(pct)) {
                        ...
                        const fill = el.querySelector('.jellyfin-on-demand-warm-bar-fill');
                        const sub = el.querySelector('.jellyfin-on-demand-warm-sub');
                        if (fill) {
    ...
    function showWarmOverlay(msg, btih) {
        ...
        el.innerHTML = `<div style="background:#1c1c1e;color:#f5f5f7;padding:1.2rem 1.5rem;border-radius:10px;box-shadow:0 12px 40px rgba(0,0,0,.5);font-size:.95rem;max-width:24rem;text-align:center;">
            <div class="jellyfin-on-demand-warm-bar-track" style="width:14rem;height:.5rem;border-radius:999px;background:rgba(255,255,255,.15);margin:0 auto .75rem;overflow:hidden;">
                <div class="jellyfin-on-demand-warm-bar-fill indeterminate" style="height:100%;width:40%;background:#56d7ff;border-radius:999px;"></div>
            </div>
            <div>${String(msg || 'Warming swarm…').replace(/</g, '&lt;')}</div>
            <div class="jellyfin-on-demand-warm-sub" style="opacity:.65;font-size:.8rem;margin-top:.4rem;">Head + tail extents — please wait</div>
        </div>`;
        if (!document.getElementById('jellyfin-on-demand-warm-keyframes')) {
            const s = document.createElement('style');
            s.id = 'jellyfin-on-demand-warm-keyframes';
            s.textContent = '@keyframes jellyfin-on-demand-spin{to{transform:rotate(360deg)}}'
                + '@keyframes jellyfin-on-demand-indeterminate{0%{margin-left:-40%;}100%{margin-left:100%;}}'
                + '.jellyfin-on-demand-warm-bar-fill{transition:width .3s linear;}'
                + '.jellyfin-on-demand-warm-bar-fill.indeterminate{animation:jellyfin-on-demand-indeterminate 1.1s ease-in-out infinite;}';
    ...
    function hijackTrackButton(btn, pickFile) {
        if (!btn || btn.dataset.jellyfinOnDemandHijacked) return;
        btn.dataset.jellyfinOnDemandHijacked = '1';
    ...
    function syncTrackButtons() {
        ...
        if (!active) {
            if (prevBtn.dataset.jellyfinOnDemandForced) {
                prevBtn.classList.add('hide');
                nextBtn.classList.add('hide');
                prevBtn.disabled = true;
                nextBtn.disabled = true;
                delete prevBtn.dataset.jellyfinOnDemandForced;
                delete nextBtn.dataset.jellyfinOnDemandForced;
            }
            return;
        }
        const { prev, next } = batchNeighbors(s);
        prevBtn.dataset.jellyfinOnDemandForced = '1';
        nextBtn.dataset.jellyfinOnDemandForced = '1';
    ...
    async function playBatchNeighbor(file) {
        ...
        try {
            ...
            if (!(attempt && attempt.ok) && typeof JE.toast === 'function') {
                JE.toast(`${attempt.message || attempt.reason || 'playback failed'}`, 8000);
