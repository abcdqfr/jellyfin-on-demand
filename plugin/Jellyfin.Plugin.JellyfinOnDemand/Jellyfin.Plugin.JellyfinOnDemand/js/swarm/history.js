// JellyfinOnDemand search history — inline browser-address-bar-style dropdown
...
(function (JE) {
    ...
    const logPrefix = 'jellyfin-on-demand:history:';
    const DROPDOWN_ID = 'jellyfin-on-demand-history-dropdown';
    const STYLE_ID = 'jellyfin-on-demand-history-styles';
    ...
    async function searchAgain(entry) {
        ...
        if (typeof JE.toast === 'function') {
            JE.toast('cannot open release picker', 4000);
        }
    }
    ...
    async function playLast(entry) {
        ...
        if (!btih) {
            if (typeof JE.toast === 'function') {
                JE.toast('no prior play for this title — search again.', 5000);
            }
            return;
        }
        const ctx = entryCtx(entry);
        ...
        if (ctx.mediaType === 'tv' && typeof JE.swarm.listFiles === 'function') {
            if (typeof JE.toast === 'function') JE.toast('reading torrent files…', 4000);
            try {
        ...
        if (typeof JE.swarmShowWarmOverlay === 'function') {
            ...
            JE.toast(`warming prior release for ${ctx.title}…`, 8000);
        }
        try {
    ...
    function wireSearchInput(input) {
        if (!input || input.dataset.jellyfinOnDemandHistoryWired) return;
        input.dataset.jellyfinOnDemandHistoryWired = 'true';
