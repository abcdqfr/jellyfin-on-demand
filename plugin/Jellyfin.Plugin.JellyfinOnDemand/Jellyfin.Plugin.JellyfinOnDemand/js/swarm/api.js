// JellyfinOnDemand control plane — Torznab search + Ensure/play-bind.
(function (JE) {
    'use strict';
    ...
    function base() {
        return ApiClient.getUrl('/JellyfinOnDemand/swarm');
    }
    ...
    api.searchTorznab = async function (query) {
        ...
        const url = ApiClient.getUrl('/JellyfinOnDemand/swarm/torznab/search', { q });
        try {
    ...
    api.formatError = function (result) {
        ...
        const map = {
            ...
            io_error: 'Cannot write the swarm cache directory (check JELLYFIN_ON_DEMAND_CACHE_DIR permissions on disk).',
            'native_error_-5': 'Cannot write the swarm cache directory (check JELLYFIN_ON_DEMAND_CACHE_DIR permissions on disk).',
