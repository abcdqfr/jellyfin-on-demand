...
(function() {
    ...
    window.JellyfinOnDemand = {
        ...
        // (/JellyfinOnDemand/cdn/{source}/{path}) — backed by an on-disk cache refreshed
        ...
        cdn: {
            ...
            url(source, path) {
                ...
                return ApiClient.getUrl(`/JellyfinOnDemand/cdn/${source}/${clean}`);
            },
    ...
    async function loadPrivateConfig() {
        try {
            const privateConfig = await ApiClient.ajax({
                type: 'GET',
                url: ApiClient.getUrl('/JellyfinOnDemand/private-config'),
                dataType: 'json'
            });
     ...
     */
     function loadSplashScreenEarly() {
        ...
        splashScript.src = ApiClient.getUrl('/JellyfinOnDemand/js/others/splashscreen.js?v=' + getScriptVersion());
        splashScript.onload = () => {
    ...
    async function initialize() {
        ...
        try {
            ...
            const fetchPromises = [
                ApiClient.ajax({ type: 'GET', url: ApiClient.getUrl(`/JellyfinOnDemand/user-settings/${userId}/settings.json?_=${Date.now()}`), dataType: 'json' })
                         .then(data => ({ name: 'settings', status: 'fulfilled', value: data }))
                         .catch(e => ({ name: 'settings', status: 'rejected', reason: e })),
                ApiClient.ajax({ type: 'GET', url: ApiClient.getUrl(`/JellyfinOnDemand/user-settings/${userId}/shortcuts.json?_=${Date.now()}`), dataType: 'json' })
                         .then(data => ({ name: 'shortcuts', status: 'fulfilled', value: data }))
                         .catch(e => ({ name: 'shortcuts', status: 'rejected', reason: e })),
                ApiClient.ajax({ type: 'GET', url: ApiClient.getUrl(`/JellyfinOnDemand/user-settings/${userId}/bookmark.json?_=${Date.now()}`), dataType: 'json' })
                         .then(data => ({ name: 'bookmark', status: 'fulfilled', value: data }))
                         .catch(e => ({ name: 'bookmark', status: 'rejected', reason: e })),
                ApiClient.ajax({ type: 'GET', url: ApiClient.getUrl(`/JellyfinOnDemand/user-settings/${userId}/elsewhere.json?_=${Date.now()}`), dataType: 'json' })
                         .then(data => ({ name: 'elsewhere', status: 'fulfilled', value: data }))
                         .catch(e => ({ name: 'elsewhere', status: 'rejected', reason: e })),
                ApiClient.ajax({ type: 'GET', url: ApiClient.getUrl(`/JellyfinOnDemand/user-settings/${userId}/hidden-content.json?_=${Date.now()}`), dataType: 'json' })
            ...
            const basePath = '/JellyfinOnDemand/js';
            const allComponentScripts = [
                ...
                // jellyfin-on-demand control plane (Torznab/Ensure — not the search chrome)
                'swarm/api.js',
                'swarm/ranker.js',
                'swarm/releases.js',
                'swarm/lucky.js',
                ...
                // jellyseerr chrome (search/posters) — data from JellyfinOnDemand/TMDB
            ...
            // JellyfinOnDemandDiscoveryEnabled (no Seerr). Do not require the legacy
