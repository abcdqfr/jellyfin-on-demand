    ...
    JE.initializeArrLinksScript = async function () {
        ...
        try {
            ...
            async function getSonarrSlugs(item) {
                ...
                try {
                    const resp = await fetch(ApiClient.getUrl(`/JellyfinOnDemand/arr/series-slugs?tvdbId=${encodeURIComponent(tvdbId)}`), {
            ...
            async function getRadarrInstances(tmdbId) {
                ...
                try {
                    const resp = await fetch(ApiClient.getUrl(`/JellyfinOnDemand/arr/movie-instances?tmdbId=${encodeURIComponent(tmdbId)}`), {
                        headers: { 'Authorization': 'MediaBrowser Token="' + ApiClient.accessToken() + '"', 'X-MediaBrowser-Token': ApiClient.accessToken() }
                    });
                    if (!resp.ok) {
