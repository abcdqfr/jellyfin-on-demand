...
namespace Jellyfin.Plugin.JellyfinOnDemand.Swarm
{
    ...
    /// </summary>
    public sealed class JellyfinOnDemandPlayBindResult
    {
    ...
    public static class SwarmErrorText
    {
        public static (string Code, string Message, int? Numeric) Describe(int? code, string? legacy = null)
        {
            ...
            return n switch
            {
                ...
                -5 => ("io_error",
                    "Cannot write the swarm cache directory (check JELLYFIN_ON_DEMAND_CACHE_DIR permissions on disk).", n),
        ...
        public static void Apply(JellyfinOnDemandPlayBindResult result)
        {
            if (string.IsNullOrEmpty(result.Error) && result.ErrorCode is null) return;
