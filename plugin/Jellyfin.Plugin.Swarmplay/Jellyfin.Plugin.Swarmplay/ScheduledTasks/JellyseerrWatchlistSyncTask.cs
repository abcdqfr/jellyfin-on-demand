...
namespace Jellyfin.Plugin.Swarmplay.ScheduledTasks
{
        ...
        private async Task<Dictionary<string, string>> GetJellyseerrUserMap(HttpClient httpClient, string jellyseerrUrl, string apiKey)
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    using var request = Jellyfin.Plugin.Swarmplay.Helpers.Jellyseerr.SeerrHttpHelper.BuildRequest(
                        HttpMethod.Get, requestUri, apiKey);
                    using var response = await httpClient.SendAsync(request);
                    var (content, error) = await Jellyfin.Plugin.Swarmplay.Helpers.Jellyseerr.SeerrHttpHelper.ReadResponseAsync(response, requestUri);
