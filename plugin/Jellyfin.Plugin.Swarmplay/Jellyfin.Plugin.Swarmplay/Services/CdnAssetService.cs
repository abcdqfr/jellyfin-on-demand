...
namespace Jellyfin.Plugin.Swarmplay.Services
{
    ...
    /// from the plugin's own <c>/Swarmplay/cdn/{source}/{path}</c> route,
    ...
    public class CdnAssetService
    {
        ...
        public CdnAssetService(Logger logger, IHttpClientFactory httpClientFactory, IApplicationPaths applicationPaths)
        {
            ...
            _cacheDir = Path.Combine(applicationPaths.PluginsPath, "configurations", "Jellyfin.Plugin.Swarmplay", "cdn-cache");
        }
        ...
        private static readonly IReadOnlyDictionary<string, CdnSource> Sources = new Dictionary<string, CdnSource>(StringComparer.Ordinal)
        {
            ...
            ["locales"] = new("https://raw.githubusercontent.com/n00bcodr/Jellyfin-Enhanced/main/Jellyfin.Plugin.Swarmplay/js/locales", Types("application/json", "text/plain")),
