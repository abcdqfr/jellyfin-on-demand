...
using Jellyfin.Plugin.Swarmplay.Helpers.Jellyseerr;
using MediaBrowser.Model.Plugins;
using MediaBrowser.Model;
using MediaBrowser.Controller.Persistence;
using Jellyfin.Plugin.Swarmplay.Model.Arr;
using Jellyfin.Plugin.Swarmplay.Extensions;
...
namespace Jellyfin.Plugin.Swarmplay.Controllers
{
    ...
    public class JellyfinEnhancedController : ControllerBase
    {
        ...
        {
            var prefix = "Jellyfin.Plugin.Swarmplay.js.locales.";
            var suffix = ".json";
        ...
        public ActionResult GetLocale(string lang)
        {
            ...
            var resourcePath = $"Jellyfin.Plugin.Swarmplay.js.locales.{sanitizedLang}.json";
            ...
            if (stream == null && sanitizedLang.Contains('-'))
            {
                ...
                var fallbackPath = $"Jellyfin.Plugin.Swarmplay.js.locales.{baseLang}.json";
        ...
        {
            var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream($"Jellyfin.Plugin.Swarmplay.{resourcePath.Replace('/', '.')}");
        ...
        {
            var allowed = Jellyfin.Plugin.Swarmplay.Helpers.ArrUrlGuard.IsAllowedUrl(url);
        ...
        {
            var allowed = await Jellyfin.Plugin.Swarmplay.Helpers.ArrUrlGuard.IsAllowedUrlAsync(url, ct).ConfigureAwait(false);
            if (!allowed && !string.IsNullOrWhiteSpace(url))
            {
                _logger.Error($"IsAllowedUrl rejected outbound URL: {url}");
            }
                ...
                if (spState != null && (spState.Series.Count > 0 || spState.Movies.Count > 0 || spState.Collections.Count > 0))
                {
                    ...
                    foreach (var kvp in items.ToList())
                    {
                        ...
                        if (sanitizeTitleStreams && stripped.StreamData != null && !stripGenresEnabled)
                        {
                            ...
                            var clonedSd = new Jellyfin.Plugin.Swarmplay.Model.TagStreamData
                            {
                                ItemName = null,
                                ItemPath = null,
                                Streams = sd.Streams?.Select(st => new Jellyfin.Plugin.Swarmplay.Model.TagMediaStream
                                {
                                    Type = st.Type,
                                ...
                                Sources = sd.Sources?.Select(_ => new Jellyfin.Plugin.Swarmplay.Model.TagMediaSource
                                {
                                    Path = null,
                                    Name = null,
                                }).ToList(),
            ...
            try
            {
                ...
                if (results != null)
                {
                    ...
                    var enrichmentTasks = results.Select(async req =>
                    {
                        ...
                        {
                            avatarUrl = $"/Swarmplay/proxy/avatar?path={Uri.EscapeDataString(avatar)}";
                        }
