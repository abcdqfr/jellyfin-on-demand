            ...
            const checkForUser = async () => {
                if (ApiClient.getCurrentUserId() && ApiClient.accessToken()) {
                    ...
                    if (isJellyseerrActive && JE.pluginConfig?.JellyseerrShowGenreDiscovery !== false) {
                        Promise.all([
                            JE.discoveryFilter?.fetchWithManagedRequest?.('/Swarmplay/tmdb/genres/tv', 'genre', {})?.catch(() => {}),
                            JE.discoveryFilter?.fetchWithManagedRequest?.('/Swarmplay/tmdb/genres/movie', 'genre', {})?.catch(() => {})
                        ]).catch(() => {});
                    }
