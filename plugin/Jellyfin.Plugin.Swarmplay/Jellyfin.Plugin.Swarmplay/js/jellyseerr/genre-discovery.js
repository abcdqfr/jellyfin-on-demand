...
(function(JE) {
    ...
    async function fetchTmdbGenres(signal) {
        ...
        try {
            ...
            const [tvResponse, movieResponse] = await Promise.all([
                fetchWithManagedRequest('/Swarmplay/tmdb/genres/tv', fetchOptions).catch(() => []),
                fetchWithManagedRequest('/Swarmplay/tmdb/genres/movie', fetchOptions).catch(() => [])
            ]);
    ...
    async function getGenreInfo(genreId, signal) {
        ...
        try {
            ...
            const response = await fetchWithManagedRequest(`/Swarmplay/genre/${genreId}`, { signal });
    ...
    async function fetchTvDiscover(genreId, page = 1, signal) {
        try {
            ...
            let path = `/Swarmplay/jellyseerr/discover/tv/genre/${genreId}?page=${page}`;
    ...
    async function fetchMovieDiscover(genreId, page = 1, signal) {
        try {
            ...
            let path = `/Swarmplay/jellyseerr/discover/movies/genre/${genreId}?page=${page}`;
            if (sortBy) path += `&sortBy=${encodeURIComponent(sortBy)}`;
            const response = await fetchWithManagedRequest(path, { signal });
            if (signal?.aborted) {
                throw new DOMException('Aborted', 'AbortError');
