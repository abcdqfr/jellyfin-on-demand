...
async function fetchRatings(tmdbId, mediaType) {
    ...
    try {
        ...
        const url = ApiClient.getUrl(`/Swarmplay/jellyseerr${endpoint}`);
        let response;
    ...
    try {
        ...
        if (JE && JE.jellyseerrAPI) {
            ...
            data = await ApiClient.ajax({
                type: 'GET',
                url: ApiClient.getUrl(`/Swarmplay/jellyseerr${endpoint}`),
                headers: { 'X-Jellyfin-User-Id': ApiClient.getCurrentUserId() },
                dataType: 'json'
            });
        }
...
async function checkForUnrequestedSeasons(data) {
    ...
    try {
        ...
        const response = await ApiClient.ajax({
            type: 'GET',
            url: ApiClient.getUrl(`/Swarmplay/jellyseerr/request?take=500&skip=0&filter=all`),
            headers: { 'X-Jellyfin-User-Id': ApiClient.getCurrentUserId() },
            dataType: 'json'
        });
...
// /Swarmplay/proxy/avatar proxy for CORS/auth) or an already-absolute URL (gravatar,
