...
(function(JE) {
    ...
    async function getStudioInfo(studioId, signal) {
        ...
        try {
            ...
            const response = await fetchWithManagedRequest(`/Swarmplay/studio/${studioId}`, { signal });
    ...
    async function searchTmdbCompany(networkName, signal) {
        ...
        try {
            ...
            const response = await fetchWithManagedRequest(
                `/Swarmplay/tmdb/search/company?query=${encodeURIComponent(networkName)}`,
                { signal }
            );
    ...
    async function fetchNetworkDiscover(networkId, page = 1, signal) {
        try {
            ...
            let path = `/Swarmplay/jellyseerr/discover/tv/network/${networkId}?page=${page}`;
    ...
    async function fetchStudioDiscover(studioId, page = 1, signal) {
        try {
            ...
            let path = `/Swarmplay/jellyseerr/discover/movies/studio/${studioId}?page=${page}`;
            if (sortBy) path += `&sortBy=${encodeURIComponent(sortBy)}`;
            const response = await fetchWithManagedRequest(path, { signal });
