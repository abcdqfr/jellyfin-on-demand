...
(function(JE) {
        ...
        const performFetch = async () => {
            ...
            try {
                const itemResult = await ApiClient.ajax({
                    type: 'GET',
                    url: ApiClient.getUrl(`/Swarmplay/watch-progress/${ApiClient.getCurrentUserId()}/${itemId}`),
                    dataType: 'json'
                });
    ...
    async function displayItemSize(itemId, container) {
        ...
        const performFetch = async () => {
            ...
            try {
                const itemResult = await ApiClient.ajax({
                    type: 'GET',
                    url: ApiClient.getUrl(`/Swarmplay/file-size/${ApiClient.getCurrentUserId()}/${itemId}`),
                    dataType: 'json'
                });
    ...
    function tmdbGet(path) {
        const url = ApiClient.getUrl(`/Swarmplay/tmdb${path}`);
    ...
    async function removeFromHomeSurface(itemId, surface, card) {
        ...
        try {
            await ApiClient.ajax({
                type: 'POST',
                url: ApiClient.getUrl(`/Swarmplay/${config.path}/hide/${itemId}`),
                data: '{}',
