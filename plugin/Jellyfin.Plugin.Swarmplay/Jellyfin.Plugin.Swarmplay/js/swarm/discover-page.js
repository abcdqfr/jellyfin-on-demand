/**
 * Swarmplay Discover pane — Seerr Discover–shaped home (trending / popular /
 ...
 * Seerr/Jellyseerr *client* UI, rolled into this Swarmplay fork (GPL-3.0 JE
 * derivative — see ATTRIBUTION.md); data comes from
 * /Swarmplay/jellyseerr/discover/* (TMDB-backed when Seerr is off — ADR-004).
 *
...
(function (JE) {
    ...
    const logPrefix = '🪼 Swarmplay Discover:';
    const PAGE_ID = 'je-discover-standalone-page';
    const NAV_CLASS = 'je-nav-discover-item';
    // Two classes on one node — query as .sections.swarmplay-discover (NOT
    // `.${SECTION_CLASS}`, which becomes ".sections swarmplay-discover" and
    // matches a descendant tag name, leaving the pane empty).
    const SECTION_CLASS = 'sections swarmplay-discover';
    const SECTION_SELECTOR = '.sections.swarmplay-discover';
    ...
    function ensureStyles() {
        if (document.getElementById('swarmplay-discover-styles')) return;
        const s = document.createElement('style');
        s.id = 'swarmplay-discover-styles';
        s.textContent = `
            .swarmplay-discover { padding: 1rem 0 3rem; }
            .swarmplay-discover .discover-page-header {
                display:flex; align-items:baseline; justify-content:space-between;
                gap:1rem; padding:0 1.25rem 1rem; flex-wrap:wrap;
            }
            .swarmplay-discover .discover-page-header h1 {
                margin:0; font-size:1.75rem; font-weight:600; letter-spacing:.02em;
            }
            .swarmplay-discover .discover-page-header .sub {
                opacity:.65; font-size:.9rem;
            }
            .swarmplay-discover .discover-slider { margin-bottom:1.75rem; }
            .swarmplay-discover .discover-slider .slider-title {
                display:inline-flex; align-items:center; gap:.4rem;
                margin:0 0 .6rem; padding:0 1.25rem;
                font-size:1.15rem; font-weight:600; color:inherit; text-decoration:none;
            }
            .swarmplay-discover .discover-slider .slider-title .material-icons {
                font-size:1.1rem; opacity:.7;
            }
            .swarmplay-discover .discover-slider .emby-scroller { padding-left:1.25rem; padding-right:1.25rem; }
            .swarmplay-discover .discover-genre-card {
            ...
            .swarmplay-discover .discover-genre-card:hover,
            .swarmplay-discover .discover-genre-card:focus {
                transform:translateY(-2px); outline:none;
                box-shadow:0 10px 24px rgba(0,0,0,.4);
            }
            .swarmplay-discover .discover-loading,
            .swarmplay-discover .discover-empty {
                padding:2rem 1.25rem; opacity:.7;
            }
            .swarmplay-discover-landing-link {
            ...
            .swarmplay-discover-landing-link:hover { background:rgba(86,215,255,.2); }
            .swarmplay-discover-landing-link .material-icons { font-size:1.1rem; }
        `;
    ...
    function mediaCardsFragment(results) {
        ...
        // Swarmplay Play/Lucky/Library branch (same as discovery-filter-utils /
        // item-details). Server /user-status already reports active for Swarmplay.
    ...
    function injectNavigation() {
        ...
        // Place Discover first in the Swarmplay section (before Bookmarks / Calendar / …),
    ...
    function injectSearchLandingLink() {
        ...
        let link = searchPage.querySelector('.swarmplay-discover-landing-link');
        if (q) {
            link?.remove();
            return;
        }
        ...
        link.type = 'button';
        link.className = 'swarmplay-discover-landing-link';
    ...
    JE.initializeDiscoverPage = function () {
        if (!discoveryEnabled()) {
            console.log(`${logPrefix} skipped (SwarmplayDiscoveryEnabled=false)`);
            return;
        }
        ensureStyles();
        injectNavigation();
